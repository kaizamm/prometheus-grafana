...
function publicConfig()
{
  ...
  read -p "[public subnet(defalut:24)]:"  publicSubnet
  if [[ ! $publicSubnet ]]; then
    publicSubnet="24"
  fi
  publicGatewayPre=`echo ${publicIp%.*}.254`
  read -p "[public gateway(default:$publicGatewayPre)]:"  publicGateway
  ...
  mv $networkDir/$publicInterface $networkDir/bak_$publicInterface$date_now  && \
...
function clusterConfig()
{
  ...
  read -p "[cluster gateway(default:$clusterGatewayPre)]:"  clusterGateway
  ...
  read -p "confirm<Enter> cancel<ctrl+c>:"
  mv $networkDir/$clusterInterface $networkDir/bak_$clusterInterface$date_now  && \
  dirNow=$(cd "$(dirname "$0")";pwd)
